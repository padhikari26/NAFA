...
class FullCalendar extends StatefulWidget {
  ...
  const FullCalendar({
    ...
    this.dateSelectedColor = Colors.white,
...
class _FullCalendarState extends State<FullCalendar> {
  ...
  void initState() {
    ...
    setState(() {
      _selectedData = widget.selectedDate;
  ...
  Widget build(BuildContext context) {
    ...
    if (firstDate.year == lastDate.year && firstDate.month == lastDate.month) {
      ...
      final index = months.indexWhere(
        (element) =>
            element!.month == _selectedData!.month &&
            element.year == _selectedData!.year,
      );
  ...
  Widget daysOfWeek(double width, String? locale, WeekDay? weekday) {
    ...
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        dayName(width / 9, daysNames[0]),
        dayName(width / 9, daysNames[1]),
        dayName(width / 9, daysNames[2]),
        dayName(width / 9, daysNames[3]),
        dayName(width / 9, daysNames[4]),
        dayName(width / 9, daysNames[5]),
        dayName(width / 9, daysNames[6]),
      ],
    );
  }
  ...
  Widget dateInCalendar(
    ...
    bool isSelectedDate = date.toString().split(" ").first ==
        _selectedData.toString().split(" ").first;
    bool isCurrentDate = date.toString().split(" ").first ==
        widget.currentDate.toString().split(" ").first;
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border:
              isCurrentDate ? Border.all(color: widget.dateSelectedBg) : null,
        ),
        alignment: Alignment.center,
        child: Column(
